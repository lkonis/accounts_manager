...
class cipher(encryption):
    ...
    def run_endec(self, *args):
        ...
        if not args:
            print '\ncipher.py - a python script for text file encryption'
            print 'the user should pick a code-word which will be used to encode a text file'
            print 'the exact same code word will be used afterward to decrypt that file'
            print '\nusage: cipher input-file code-word\n'
            print 'by default, infile will be encoded (encrypted)'
            print 'if it has a ".coded" extention, then it will be decoded'

            sys.exit(1)

        filename = " ".join(args[:-1])  # file to be encoded/decoded
        password = args[-1]  # password entered by the user
        if len(password) <= 4:
            print ("Cipher: no password or password too short (min 4 char)\nLeaving file unencoded")
            sys.exit(2)
        ...
        if self.mode == 'ENC':  # to encode
            ...
            else:  # todo - can never reach here
